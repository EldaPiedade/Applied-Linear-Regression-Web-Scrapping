...
2) Model 8:	 y ~ x3 + x7
3) Model 24: y ~ x5 + x7
4) Model 64: y ~ x2 + x3 + x6 + x7
5) Model 99: y ~ x1 + x3 + x5 + x6 + x7
6) Model 122:y ~ x1 + x2 + x3 + x4 + x5 + x7
7) Model 127:y ~.
...
The model with the lowest PRESS value is model 99 [ log(y) ~ x1 + x3 + x5 + x6 + x7] however there is evidence of multicolliniearity.

The model with the second lowest PRESS value is model 64 [ log(y) ~ x2 + x3 + x6 + x7] and the same model doesnt show any evidence of multicolinearity in the variance inflation factor test of each regressor.
